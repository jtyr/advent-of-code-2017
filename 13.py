...
def walk(fw, delay=0, exit=False):
    scanner = delay
    severity = 0
    caught = False

    for pos in range(max(fw.keys()) + 1):
        if pos in fw and scanner % (fw[pos] * 2 - 2) == 0:
            severity += pos * fw[pos]
...
def main():
    ...
    severity, _ = walk(dict(fw))

    while True:
        _, caught = walk(fw, delay, True)
    ...
    print("[Star 1] Severity: %d" % severity)
    print("[Star 2] Delay: %s" % delay)
