...
pixelated hurricane: a deadly spinlock. This spinlock isn't just
...
steps forward from there the same number of steps, and wherever it stops,
inserts after it the second new value, 2, and uses that as the new
...
def spin1(stop_at, step):
    buffer = []
    pos = 0

    for n in xrange(stop_at + 1):
        buffer.insert(pos, n)
        pos = (pos + step) % len(buffer) + 1

    return buffer[buffer.index(stop_at) + 1]


def spin2(stop_at, step):
    pos = 0
    val = None
...
def main():
    ...
    step = int(get_data(sys.argv[1]))

    val1 = spin1(2017, step)
    val2 = spin2(50000000, step)

    print("[Star 1] Value after 2017: %d" % val1)
    print("[Star 2] Value after 0: %d" % val2)
