...
def loop(lengths, nums, num_length, pos, skip):
    for l in lengths:
...
def main():
    ...
    if 'test1' in sys.argv[1]:
        test = 1
    elif 'test' in sys.argv[1] and 'test1' not in sys.argv[1]:
        test = 2
    else:
        test = 0

    data = get_data(sys.argv[1]).rstrip()
    num_length = 5 if test == 1 else 256
    ...
    if test != 2:
        ...
        pos, skip = loop(lengths, nums, num_length, pos, skip)

        print("[Star 1] Multiplied first two elements: %d" % (nums[0]*nums[1]))
    ...
    if test != 1:
        ...
        print("[Star 2] Knot Hash: %s" % dense_hash)
