...
def main():
    ...
    data_len = len(data)
    data_len_half = int(data_len/2)
    my_sum1 = 0
    my_sum2 = 0

    for i, n in enumerate(data):
        next_i1 = i + 1
        next_i2 = i + data_len_half

        if i + 1 == data_len:
            next_i1 = 0

        if i + data_len_half >= data_len:
            next_i2 = data_len_half - data_len + i

        if data[i] == data[next_i1]:
            my_sum1 += int(data[i])

        if data[i] == data[next_i2]:
            my_sum2 += int(data[i])

    print("[Star 1] Sum: %d" % my_sum1)
    print("[Star 2] Sum: %d" % my_sum2)
