...
def main():
    ...
    nums = map(int, re.split('\s+', data))
    max_idx = len(nums) - 1
    hist = []
    cycles1 = 0
    cycles2 = 0

    hist.append(','.join(map(str, nums)))
    ...
    while True:
        ...
        # Distribute the bank
        for i in range(nums[n_max_idx] + 1):
            if i == 0:
                nums[idx] = 0
            else:
                nums[idx] += 1
        ...
        checksum = ','.join(map(str, nums))

        if checksum in hist:
            cycles2 = len(hist) - hist.index(checksum)

            break
        else:
            hist.append(checksum)

    print("[Star 1] Cycles: %d" % cycles1)
    print("[Star 2] Cycles: %d" % cycles2)
