...
run the code yourself. Unfortunately, you don't see any documentation, so
...
it in b, and then each receives the program ID of the other program
...
def main():
    ...
    # Parse the program
    for line in map(lambda x: x.rstrip(), data):
        parts = line.split(' ')
        rec = {
            'ins': parts[0],
            'reg': parts[1],
        ...
        if len(parts) == 3 and parts[2] in AZ:
            rec['val'] = parts[2]
        elif len(parts) == 3:
            rec['val'] = int(parts[2])
    ...
    while True:
        ...
        if p['ins'] == 'snd':
            print "snd %d" % get(regs, p)
            played = get(regs, p)
        elif p['ins'] == 'set':
            print "ins %s %d" % (p['reg'], get(regs, p))
            regs[p['reg']] = get(regs, p)
        elif p['ins'] == 'add':
            print "add %s %d" % (p['reg'], get(regs, p))
            regs[p['reg']] += get(regs, p)
        elif p['ins'] == 'mul':
            print "mul %s %d" % (p['reg'], get(regs, p))
            regs[p['reg']] *= get(regs, p)
        elif p['ins'] == 'mod':
            print "mul %s %d" % (p['reg'], get(regs, p))
            regs[p['reg']] %= get(regs, p)
        elif p['ins'] == 'rcv' and regs[p['reg']] > 0:
            print "rcv %d" % get(regs, p)
            break
        elif p['ins'] == 'jgz' and regs[p['reg']] > 0:
            print "jgz %s %d" % (p['reg'], get(regs, p))
            pos += get(regs, p) - 1
        else:
            print '-', p['ins']
    ...
    print("[Star 1] Last played sound: %d" % played)
    #print("[Star 2] Number of values sent by prog 1: %d" % sent)
